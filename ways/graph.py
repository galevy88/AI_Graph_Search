...
class Roads(dict):
    ...
    def go_to_next_junction(self, current_junction_index):
        junctions = self.junctions()
        current_junction = junctions[current_junction_index]
        if len(current_junction.links) > 0:
            r = random.randint(0, len(current_junction.links) - 1)
            target_index = current_junction.links[r].target
        else:
            return -1
        return target_index
